...
export default function ProjectList({optimizedImages}: ProjectListProps) {
    ...
    // Embla drives a horizontal carousel on desktop and a vertical one on
    // mobile (one card at a time either way).
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        // containScroll defaults to "trimSnaps", which mis-measures this
        // vertical, viewport-sized-slide layout and collapses all slides
        // into a single scroll snap; disabling it fixes navigation.
        const api = EmblaCarousel(viewportEl, {
            loop: true,
            align: "start",
            axis: isDesktop ? "x" : "y",
            containScroll: false,
    ...
    // Independent of selectedIndex, so slide changes (including autoplay
    // ticks) don't re-diff all 5 cards just to move the active dot.
    const cardItems = useMemo(() => projects.map((project: IProject, index: number) => {
        const isExternal = !!project.url && EXTERNAL_URL_RE.test(project.url);
        ...
        return (
                <li key={project.id} class='shrink-0 grow-0 basis-full md:basis-[98%] md:pr-6'>
                    <Wrapper
                            ...
                            class='flex flex-col md:flex-row items-stretch gap-8 md:gap-6 rounded-3xl p-8 md:p-20 h-140 md:h-145 overflow-hidden bg-(--accent-light) dark:bg-(--accent-black)'
                    >
                        ...
                        <img
                                ...
                                alt={project.name}
                                class={`w-lg m-auto md:w-1/2 h-full flex-1 md:flex-none object-cover rounded-3xl border-2 border-gray-600 dark:border-gray-200 shadow-2xl dark:shadow-lg project-image-float ${project.accent.shadow_class}`}
                                // @ts-ignore
    ...
    return (
            <div id='projects-list' class='w-[90%] mx-auto pb-16 md:pb-0'>
                <div class='overflow-hidden h-140 md:h-auto' ref={viewportRef}>
                    <ul class='flex flex-col md:flex-row gap-4 md:gap-0 touch-pan-x md:touch-pan-y' role='list'>
                        {cardItems}
                    </ul>
                </div>
                ...
                <div class='flex items-center justify-center gap-6 mt-8'>
                    ...
                    >
                        <svg xmlns='http://www.w3.org/2000/svg'
                             viewBox='0 0 256 256'
                             class='w-5 h-5 rotate-90 md:rotate-0'
                             fill='currentColor'
                             aria-hidden='true'>
